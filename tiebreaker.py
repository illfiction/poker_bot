...
def straight_tiebreaker(user_hand, opp_hand):
    ...
    user_straight_high_card = 0
    opp_straight_high_card = 0

    for i in range(len(user_ranks)-4):
        if user_ranks[i+4] - user_ranks[i] == 4:
            user_straight_high_card = user_ranks[i+4]
    if {2,3,4,5,14}.issubset(user_ranks) and user_straight_high_card == 0:
        user_straight_high_card = 5

    for i in range(len(opp_ranks)-4):
        if opp_ranks[i+4] - opp_ranks[i] == 4:
            opp_straight_high_card = opp_ranks[i+4]
    if {2,3,4,5,14}.issubset(opp_ranks) and opp_straight_high_card == 0:
        opp_straight_high_card = 5
...
if __name__ == '__main__':
    ...
    print(straight_tiebreaker(user_hand, opp_hand))
